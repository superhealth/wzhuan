...
<body>
	<form action="articlecategory/${msg }.do" name="Form" id="Form" method="post">
		...
		<table>
			...
			<tr>
				<td>
				<select name="PARENT" id="PARENT" value="${pd.PARENT}" title="上级分类">
					<option value=""> 顶级分类</option>
					<c:forEach items="${articlecates}" var="var" varStatus="vs">
						<option value="${var.ARTICLECATEGORY_ID}" <c:if test="${var.ARTICLECATEGORY_ID == pd.PARENT}">selected</c:if>>
				...
				<!--<td><input type="number" name="GRADE" id="GRADE" value="${pd.GRADE}" maxlength="32" placeholder="这里输入上级分类" title="上级分类"/></td>  -->
			</tr>
			<tr>
